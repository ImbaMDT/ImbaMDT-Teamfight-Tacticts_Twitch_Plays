...
class Read_MEM:
    ...
    def __int__(self):
        self.mem = Pymem(constants.PROCESS_NAME)

    def champ_pointers(self):
        # Given constants.Process_Name
        # returns Object Name and Pos
        field_offset = self.mem.read_int(mem.base_address + constants.oTFTObject)
        ...
        for x in range(9):  # max loops
            ...
            try:
                ...
                if field_object_bytes == b'Obje':  # player_name is too large and a pointer now
                    ...
                    player_y = round(self.mem.read_float(field_iter + constants.oY) + selfmem.read_float(field_iter + constants.oY2))
                    player_id = str(player_x + player_y)
                    player_offsets.extend((player_id, player_name))
